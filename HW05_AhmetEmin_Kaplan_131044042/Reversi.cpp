...
Cell Reversi::operator()(int corY,const char* corX)const
{
    int x=-1;
    --corY;
    int temp=1;
    ...
    while(corX[i]!='\0')
    {
        ...
        chars.push_back(corX[i]);
        ++i;
    } 
    
    for(int j=chars.size()-1;j>=0;--j)
    {
        x+=(((int)chars[j])-96)*temp;
        temp=temp*26;
    }
    char id;
...
/*User icin hamle yapan operatordur alinan hucrenin kordinatlarina gore hamle yapar*/
/*Hamle yapilmadan onceki durumu previous_moves vectorune eleman olarak ekler*/
Reversi Reversi::operator+=(const Cell& move)
{
   previous_moves.push_back(*this);
    int i,j;
...
/*Hamle yapilmadan onceki durumu previous_moves vectorune eleman olarak ekler ve
 computer icin hamle yapar*/
...
Cell Reversi::operator[](const char* cellname)const
{
    vector<char> numbers;
    vector<char> chars;
    int i=0;
    while(cellname[i]!='\0')
    {   
        if(isdigit(cellname[i]))
            numbers.push_back(cellname[i]);
            
        else
            chars.push_back(cellname[i]);
    ...
    {
        y+=((numbers[numbers.size()-i-1]-'0')*temp);
        temp=temp*10;
        //cout<<x<<"-----\n";
    }
    temp=1;
    ...
    {
        x+=(((int)(chars[numbers.size()-j-1]))-96)*temp;
        temp=temp*26;
    }    
...
/*Her iki tarafin hamle yapma aninda kaydedilen onceki durumlar previous_moves adli
 Reversi vectorunde kaydedilmekteydi ve o vectordeki elemanlari kullanarak 
 * oyunun baslangicina kadar gidebilme imkanimiz olusuyor*/
Reversi Reversi::operator--()
{
    int l;
    ...
    {
        l=previous_moves[previous_moves.size()-1].gameCells[0].size();
        ...
        for(int z=0;z<l;++z)
        tempvector.push_back(previous_moves[previous_moves.size()-1].gameCells[0][z]);
        ...
        cout<<"Oyun zaten baslangic hamlesinde!\n";
